...
interface UseGameLogicOptions {
  ...
  setCardEffectDisplay: React.Dispatch<
    React.SetStateAction<Record<number, { emoji: string; hideTime: number }>>
  >;
  eventCounts: Record<number, Record<number, number>>;
  setEventCounts: React.Dispatch<
...
export function useGameLogic(opts: UseGameLogicOptions) {
  ...
  const calculateNewPosition = useCallback(
    (player: Player, steps: number): { pos: number; lap: number } => {
  ...
  const animateDiceRoll = useCallback(
    (results: number[]): Promise<void> => {
      return new Promise<void>((resolve) => {
        if (!(window as any).gsap) {
          resolve();
          return;
        }
        const promises = diceRefs.current.slice(0, diceCount).map(
          (el, idx) =>
    ...
    if (isMultiplayer) {
      ...
      if (cp && (cp.skipTurn || cp.finished)) {
        if (cp.skipTurn) {
          ...
          if (ncd[turn]) {
            ncd[turn].hideTime = Date.now() + 1000;
    ...
    try {
      if (isMultiplayer && roomId) {
        ...
        if ((window as any).gsap && r?.length > 0) {
          ...
          setDiceResults(r);
          addLog(
            `Player ${turn + 1} rolled ${diceCount === 1 ? v : `${r.join(", ")} (总计: ${v})`}`,
          );
          setIsRolling(false);
          handleMove(v);
        } else {
        ...
        if ((window as any).gsap) {
          ...
          addLog(
            `Player ${turn + 1} rolled ${diceCount === 1 ? total : `${results.join(", ")} (总计: ${total})`}`,
          );
          setIsRolling(false);
          handleMove(total);
        }
  ...
  const executeCardEffect = useCallback(
    (card: Card, targetId: number | null = null) => {
      ...
      setHasUsedCard(true);
      if (isMultiplayer && roomId)
        roomUseCard({ card, playerUpdates }).catch(console.error);
      if (card.target === "SELF" && card.effect.move)
        animatePieceMove(turn, () => setIsMoving(false));
    },
    [
      players,
      turn,
