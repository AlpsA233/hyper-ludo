    ...
    <div className="fixed inset-0 bg-gradient-to-br from-yellow-400/20 via-orange-500/20 to-red-500/20 backdrop-blur-md flex items-center justify-center p-4 z-[110]">
      <div
        ...
        <div className="animate-bounce">
          <Trophy className="mx-auto text-yellow-300" size={80} strokeWidth={1.5} />
        </div>
        ...
        <div className="flex items-center justify-center gap-4">
          <div
            ...
            style={{ background: championColor, borderColor: championColor, boxShadow: `0 0 30px ${championColor}` }}>
            {champion.avatar?.startsWith("http") ? (
              <img src={champion.avatar} alt={champion.name} className="w-full h-full rounded-full object-cover" />
            ) : (
          ...
          <div className="text-left">
            ...
            <p className="text-lg text-yellow-200">{t.victoryAchieved || "第一名完赛！"}</p>
          </div>
        </div>
        ...
        {ranked.length > 1 && (
          <div className="space-y-2 text-left">
            {ranked.slice(1).map((p, i) => {
              ...
              return (
                <div key={p.id} className="flex items-center gap-3 px-4 py-2 rounded-xl bg-white/10">
                  ...
                  {isLast && (
                    <span className="text-xs text-red-300 font-bold">末位淘汰</span>
                  )}
                </div>
              );
            })}
