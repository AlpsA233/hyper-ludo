  ...
  const applyEventEffect = async () => {
    ...
    const advanceTurn = async (updates: any[]) => {
      if (!isMultiplayer || !roomId) {
        ...
        await syncEventAndEndTurn(updates);
      }
      setIsMoving(false);
      setActiveEvent(null);
      setHasUsedCard(false);
    ...
    if (activeEvent.type === "MOVE" && activeEvent.val !== 0) {
      ...
      addLog(
        `Event: ${affected.map((i) => `Player ${i + 1}`).join(", ")} moved ${activeEvent.val > 0 ? "+" : ""}${activeEvent.val}`,
      );
      ...
      addLog(
        `Event: ${affected.map((i) => `Player ${i + 1}`).join(", ")} will skip next turn`,
      );
      setPhase("playing");
      await advanceTurn(
        affected.map((idx) => ({ playerIndex: idx, skipTurn: true })),
      );
      ...
      addLog(
        `Event: ${affected.map((i) => `Player ${i + 1}`).join(", ")} returned to lap start`,
      );
      animatePieceMove(affected[0] ?? turn, async () => {
        setPhase("playing");
        await advanceTurn(newPositions);
